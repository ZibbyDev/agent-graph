...
import type { Graph, Locator } from '../../src/index.js';
 ...
 */
export async function hub(g: Graph, c: ReturnType<typeof clock>) {
  const put = (id: string, kind: string) => g.put({ id, kind, label: id, provenance: 'observed' });
  await put('project:p', 'project');
  await put('repo:r', 'repo');
  for (let i = 1; i <= 20; i++) await put(`ticket:${i}`, 'ticket');
  await put('file:a', 'file');
  await put('file:b', 'file');
  await g.link({ src: 'project:p', dst: 'repo:r', rel: 'has_repo', cost: 10, provenance: 'observed' });
  for (let i = 1; i <= 20; i++) await g.link({ src: 'repo:r', dst: `ticket:${i}`, rel: 'contains', cost: 10, provenance: 'observed' });
  ...
  c.tick();
  const t1a = await g.link({ src: 'ticket:1', dst: 'file:a', rel: 'touched', cost: 1, provenance: 'observed' });
  const t7a = await g.link({ src: 'ticket:7', dst: 'file:a', rel: 'touched', cost: 1, provenance: 'observed' });
  c.tick();
  const t1b = await g.link({ src: 'ticket:1', dst: 'file:b', rel: 'touched', cost: 1, provenance: 'observed' });
  const t7b = await g.link({ src: 'ticket:7', dst: 'file:b', rel: 'touched', cost: 1, provenance: 'observed' });
  const t7p = await g.link({ src: 'ticket:7', dst: 'project:p', rel: 'belongs_to', cost: 1, provenance: 'claimed' });
  const dup = await g.link({ src: 'ticket:1', dst: 'ticket:7', rel: 'duplicate_of', cost: 5, provenance: 'claimed' });
  return { t1a, t7a, t1b, t7b, t7p, dup };
}

export function recallSuite(h: Harness): void {
  /** The root is privileged (so tests can derive other-origin handles) and
   *  hence trusted (the hub is 'observed'). */
  async function fixture() {
    const c = clock();
    const g = await h.open({ origin: 'run:a', privileged: true, now: c.now });
  ...
  describe('recall — weighted walk', () => {
    ...
    test('multi-seed: each hit reports the seed it was reached from; unknown seeds are skipped', async () => {
      ...
      for (const hit of r.hits) assert.equal(hit.seed, 'ticket:1');
    ...
    test('undirected edges traverse either way at the same cost, in any direction mode', async () => {
      const c = clock();
      const g = await h.open({ origin: 'run:a', trusted: true, now: c.now });
      for (const id of ['a', 'b', 'c']) await g.put({ id, kind: 'k', label: id, provenance: 'observed' });
      await g.link({ src: 'a', dst: 'b', rel: 'peer', directed: false, cost: 1, provenance: 'observed' });
      await g.link({ src: 'c', dst: 'b', rel: 'points', directed: true, cost: 1, provenance: 'observed' });
    ...
    test('ties: equal cost resolves to the earlier-recorded edge into the node', async () => {
      const c = clock();
      const g = await h.open({ origin: 'run:a', trusted: true, now: c.now });
      for (const id of ['s', 'x', 'y', 't']) await g.put({ id, kind: 'k', label: id, provenance: 'observed' });
      ...
      await g.link({ src: 's', dst: 'x', rel: 'r', provenance: 'observed' });
      c.tick();
      await g.link({ src: 's', dst: 'y', rel: 'r', provenance: 'observed' });
      c.tick();
      const yt = await g.link({ src: 'y', dst: 't', rel: 'r', provenance: 'observed' });
      c.tick();
      await g.link({ src: 'x', dst: 't', rel: 'r', provenance: 'observed' });
      const r = await g.recall({ seeds: ['s'], maxCost: 2 });
      const t = r.hits.find((hit) => hit.node.id === 't')!;
  ...
  describe('recall — time axes', () => {
    test('asOf shows the historical view before a supersede', async () => {
      const { g, c, edges } = await fixture();
      const before = c.t;
      c.tick();
      await g.supersede(edges.t7p.id, { src: 'ticket:7', dst: 'repo:r', rel: 'belongs_to', cost: 1, provenance: 'claimed' });
    ...
    test('validAt excludes an edge whose validTo has passed and one whose validFrom is ahead', async () => {
      const c = clock();
      const g = await h.open({ origin: 'run:a', trusted: true, now: c.now });
      for (const id of ['a', 'b', 'd']) await g.put({ id, kind: 'k', label: id, provenance: 'observed' });
      await g.link({ src: 'a', dst: 'b', rel: 'on_call', validFrom: 100, validTo: 200, provenance: 'observed' });
      await g.link({ src: 'a', dst: 'd', rel: 'on_call', validFrom: 200, validTo: null, provenance: 'observed' });
    ...
    test('asOf applies to nodes: the version current then, and nothing that did not exist yet', async () => {
      const g = await h.open({ origin: 'run:a', trusted: true, now: () => 1000 });
      await g.put({ id: 's', kind: 'k', label: 'seed', recordedAt: 100 });
      await g.put({ id: 'n', kind: 'k', label: 'old label', attrs: { v: 1 }, recordedAt: 100 });
      await g.link({ src: 's', dst: 'n', rel: 'r', recordedAt: 100 });
      await g.put({ id: 'n', kind: 'k', label: 'new label', attrs: { v: 2 }, recordedAt: 200 });
      await g.put({ id: 'late', kind: 'k', label: 'born at 300', recordedAt: 300 });
      await g.link({ src: 'late', dst: 'n', rel: 'r', recordedAt: 300 });
    ...
    test('validFrom omitted = unbounded past: an edge recorded at 200 holds at validAt 50', async () => {
      ...
      await g.link({ src: 'a', dst: 'c', rel: 'r', validFrom: 200 });
    ...
    test('recordedBetween window with null bounds', async () => {
      const c = clock();
      const g = await h.open({ origin: 'run:a', trusted: true, now: c.now });
      for (const id of ['a', 'b', 'd', 'e']) await g.put({ id, kind: 'k', label: id, provenance: 'observed' });
      const t0 = c.t;
      await g.link({ src: 'a', dst: 'b', rel: 'r', provenance: 'observed' });
      c.tick();
      const t1 = c.t;
      await g.link({ src: 'a', dst: 'd', rel: 'r', provenance: 'observed' });
      c.tick();
      const t2 = c.t;
      await g.link({ src: 'a', dst: 'e', rel: 'r', provenance: 'observed' });
  ...
  describe('recall — filters', () => {
    test('scope: null-scope edges are global, scoped edges need to be asked for', async () => {
      const c = clock();
      const g = await h.open({ origin: 'run:a', trusted: true, now: c.now });
      for (const id of ['a', 'g', 'm', 'f']) await g.put({ id, kind: 'k', label: id, provenance: 'observed' });
      await g.link({ src: 'a', dst: 'g', rel: 'r', provenance: 'observed' });
      await g.link({ src: 'a', dst: 'm', rel: 'r', scope: 'main', provenance: 'observed' });
      await g.link({ src: 'a', dst: 'f', rel: 'r', scope: 'feature/x', provenance: 'observed' });
    ...
    test('order recent / oldest by node recordedAt', async () => {
      const c = clock();
      const g = await h.open({ origin: 'run:a', trusted: true, now: c.now });
      await g.put({ id: 's', kind: 'k', label: 's', provenance: 'observed' });
      for (const id of ['old', 'mid', 'new']) {
        c.tick();
        await g.put({ id, kind: 'k', label: id, provenance: 'observed' });
      }
      // Link in reverse so cost/insertion order cannot masquerade as time order.
      for (const id of ['new', 'mid', 'old']) await g.link({ src: 's', dst: id, rel: 'r', provenance: 'observed' });
      const recent = await g.recall({ seeds: ['s'], order: 'recent' });
      assert.deepEqual(recent.hits.map((hit) => hit.node.id), ['new', 'mid', 'old']);
      const oldest = await g.recall({ seeds: ['s'], order: 'oldest' });
      assert.deepEqual(oldest.hits.map((hit) => hit.node.id), ['old', 'mid', 'new']);
    });

    test('match and locate as entry points; locate without a locator or embedding throws', async () => {
      const { g } = await fixture();
      ...
      await assert.rejects(g.recall({ locate: 'marketplace' }), /no embedding \(or locator\)/);
  ...
  describe('recallMany', () => {
    const nodeIds = (r: { hits: Array<{ nodeId: string }> }) => r.hits.map((hit) => hit.nodeId).sort();
    ...
    test('a node reached by two queries is serialised exactly once', async () => {
      ...
      assert.ok(r.results.every((q) => q.hits.some((hit) => hit.nodeId === 'file:a')), 'both queries reach file:a');
  ...
  describe('subgraph', () => {
    test('returns the induced subgraph: cross-edges between reached nodes that no recall path contains', async () => {
      const { g, edges } = await fixture();
      const recall = await g.recall({ seeds: ['ticket:1'], maxCost: 3 });
      const pathEdgeIds = new Set(recall.hits.flatMap((hit) => hit.path.map((e) => e.id)));
    ...
    test('honours the same filters as recall and does not bump access', async () => {
      ...
      c.tick();
      await g.supersede(edges.t7b.id);
  ...
  describe('access counts', () => {
    test('every edge in every returned recall path is counted; subgraph never counts; a read-only handle records nothing', async () => {
      const c = clock();
      const g = await h.open({ origin: 'run:a', privileged: true, now: c.now });
      const edges = await hub(g, c);
      await g.subgraph({ seeds: ['ticket:1'], maxCost: 3 });
      assert.deepEqual((await g.dump()).access, [], 'subgraph is an export, not a retrieval');
      const ro = g.as('reader', { readOnly: true });
      await ro.recall({ seeds: ['ticket:1'], maxCost: 2 });
      await ro.as('reader').recall({ seeds: ['ticket:7'], maxCost: 2 });
      assert.deepEqual((await g.dump()).access, [], 'a read must not write');

      await g.recall({ seeds: ['ticket:1'], maxCost: 2 });
      // Paths: t1a, t1b, t1a+t7a → t1a twice, t1b once, t7a once.
      const rows = (await g.dump()).access;
